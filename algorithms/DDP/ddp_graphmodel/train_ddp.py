...
class Dataset:
    def __init__(self):
        ...
        self.data = torch.load('/home/mcd1619/MB-MARL/algorithms/mbdppo/data.pt')
...
class GraphConvolutionalModel(nn.Module):
    ...
    def forward(self, s, a, r, s1, d, length = 1):
        ...
        """        
        Input shape:s,a,r,s1,d:[256 * 4 * 28 * 10]
        """
...
class Model(nn.Module):
    def __init__(self,adj,observation_dim,action_dim,n_agent,args,lr):
        super().__init__()
        # ddp部分
        self.adj = adj
        self.args = args
        ...
        self.model = GraphConvolutionalModel(adj, observation_dim, action_dim, n_agent)
        self.model.to(self.device)
        # print(self.device)
    ...
    def updateModel(self, length=1):
        ...
        #train模型
            ...
            for step, train_batch in enumerate(self.train_dataloader): 
                ...
                # ss = ss[0].to(self.device)
                # actions = actions[0].to(self.device)
                # rs = rs[0].to(self.device)
                # s1s = s1s[0].to(self.device)
                # ds = ds[0].to(self.device)
            ...
            if self.args.local_rank == 0:
                if train_loss < min_train_loss:
                    save_file = '%s/model_ddp.pth' % (self.args.save_path)
...
if __name__ == "__main__":
    ...
    def get_args():
        ...
        # 数据集加载过程中多线程参数，参数越大，数据加载越快，可根据cpu核心数调整
        parser.add_argument("--workers", type=int, default=1, help="num woekers")
        parser.add_argument("--save_path", type=str, default="/home/mcd1619/MB-MARL/algorithms/mbdppo/checkpoints", help="model save path")
        parser.add_argument("--epochs", type=int, default=1, help="mutil train model")
    ...
    #需要传入的数据 length,logger,adj,observation_dim,action_dim,n_agent,p_args,data,lr
    #-----------------------------------------------------------------------   
    # data = Dataset()
    observation_dim = 10
    action_dim = 1
    ...
    adj = np.zeros((n_agent, n_agent))
    for i in range(n_agent):
        adj[i][i] = 1
        adj[i][(i+1)%n_agent] = 1
        adj[i][(i+n_agent-1)%n_agent] = 1

    adj = torch.as_tensor(adj, device='cpu', dtype=torch.float)
...
#---------------------------------------------------------------------------------------
    ...
    model = Model(adj,observation_dim,action_dim,n_agent,args,lr)   
    
    
    #model.to('cuda')
    model.updateModel(length)
